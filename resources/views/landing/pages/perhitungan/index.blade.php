...
@section('content')
    ...
    <section id="perhitungan" style="margin-top: 30px">
        <div class="container">
            <ul class="nav nav-pills" id="" role="tablist">
                @foreach ($penyakits as $sick)
                    <li class="nav-item btn-penyakit">
                        <a class="nav-link  tab-penyakit" id="home-tab3" data-toggle="tab" href="#home3" role="tab"
                            aria-controls="home" aria-selected="true" data-id="{{ $sick->id }}">{{ $sick->nama }}</a>
                    </li>
                @endforeach
            </ul>

            <button class="btn btn-outline-primary" id="buttonForm" style="margin: 30px 0;">Additional Option</button>
    ...
    <script>
                ...
                success: function(response) {
                    ...
                    output.forEach((plant, index) => {
                        html += '<tr class="rowRanking">' +
                            '<td>' + (index + 1) + '</td>' +
                            "<td><a href='/tumbuhan/detail/" + plant.slug + "'>" +
                            plant.produk + '</a></td>' +
        ...
        $(document).ready(function() {
            ...
            $('#buttonForm').on('click', function() {
                var jumlahActive = $('.btn-penyakit.active').length;
                if (jumlahActive > 0) {
